...
class AdaptiveFrameSampler:
    ...
    def calculate_sampling_interval(self, fps, analysis_type, content_complexity=None):
        ...
        if content_complexity == 'high':
            ...
            # 짧은 비디오의 경우 더 많은 프레임 처리
            base_interval = min(int(fps * 0.5), base_interval)  # 0.5초마다 처리
...
class EnhancedVideoAnalyzer:
    ...
    def detect_and_analyze_persons_advanced(self, frame, frame_id, timestamp, context_info=None):
        ...
        try:
            ...
            print(f"      🎯 YOLO 검출 시작 (신뢰도 레벨: {confidence_levels})")
            
            for conf_level in confidence_levels:
                # 모든 클래스 감지 (person, bag, car, bicycle 등)
                results = self.model(frame, verbose=False, conf=conf_level)  # 모든 클래스 감지
                        ...
                        for box, conf, class_id in zip(boxes, confidences, class_ids):
                            ...
                            if object_crop.size > 0:
                                ...
                                if class_name == 'person':
                                    ...
                                    print(f"        🔍 {class_name} 감지 (신뢰도: {conf:.2f})")
    ...
    def analyze_video_comprehensive_advanced(self, video, analysis_type='enhanced', progress_callback=None):
        ...
        try:
            ...
            print(f"   - 예상 처리 프레임: {max_frames_to_process}")
            print(f"   - 비디오 길이: {duration:.1f}초")
            print(f"   - 예상 분석 시간: {max_frames_to_process * 2:.1f}초 (추정)")
            print("=" * 60)
            print()
            ...
            while cap.isOpened() and processed_frames < max_frames_to_process:
                ...
                print(f"🔄 프레임 처리 중... ({processed_frames + 1}/{max_frames_to_process}) - {progress_percent:.1f}%")
                ...
                try:
                    ...
                    if (current_time - last_progress_time >= 2.0) or (processed_frames % max(1, (total_frames // sample_interval) // 10) == 0):
                        ...
                        print(f"🔄 분석 진행률: {progress_percent:.1f}% ({processed_frames}개 프레임 처리)")
                        print(f"   - 현재 프레임: {frame_id}/{total_frames}")
                        print(f"   - 감지된 사람: {len(detected_persons)}명")
            ...
            print(f"   - PersonDetection 저장: {len(person_database)}개")
            print(f"   - YOLO 객체 감지: 활성화")
            print(f"   - 품질 점수: {quality_metrics.get('overall_quality', 0):.2f}")
            print(f"   - 분석 속도: {processed_frames/processing_time:.1f} 프레임/초")
            print(f"   - 평균 프레임당 처리 시간: {processing_time/processed_frames:.2f}초")
            print("=" * 60)
            print()
    ...
    def _save_person_detections_to_db(self, video, frame, detected_persons, frame_id, timestamp):
        """PersonDetection 데이터와 YOLO 객체 데이터를 데이터베이스에 저장"""
        try:
            ...
            for result in results:
                if result.boxes is not None:
                    for box in result.boxes:
                        ...
                        # 디버깅: 모든 감지된 객체 로그 출력
                        if confidence > 0.05:  # 5% 이상이면 로그 출력
                            print(f"🔍 YOLO 감지: {class_name} (신뢰도: {confidence:.1%})")
                        
                        # 신뢰도 1% 이상만 저장 (가방 등 작은 객체 감지 향상)
                        if confidence > 0.01:
        ...
        try:
            ...
            PersonSearchIndex.objects.create(
                ...
                visual_features_index={
                    ...
                    'body_type': 'unknown',  # 향후 구현
                    'hair_style': 'unknown',  # 향후 구현
                    'facial_features': []  # 향후 구현
                }
            )
            
        except Exception as e:
            print(f"⚠️ PersonSearchIndex 생성 오류: {e}")
    ...
    def _estimate_height_from_bbox(self, person_detection):
        ...
        try:
            ...
            if height_ratio > 0.7:  # 화면의 70% 이상
                return 'tall'
            elif height_ratio < 0.3:  # 화면의 30% 미만
                return 'short'
            else:
                return 'medium'
        except:
